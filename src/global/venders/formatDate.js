...
afterMonthDate.setMonth(afterMonthDate.getMonth() + 1);
var afterMonth = afterMonthDate.getMonth();
//格式化日期：yyyy-MM-dd
...
export function getLastMonthStartDate() {
  ...
  if (afterMonth == 11) {
    afterMonthStartDate = new Date(nowYear - 1, afterMonth, 1);
  } else {
...
export function getLastMonthEndDate() {
  ...
  if (afterMonth == 11) {
    afterMonthEndDate = new Date(
      nowYear - 1,
      afterMonth,
      getMonthDays(afterMonth)
    );
  } else {
