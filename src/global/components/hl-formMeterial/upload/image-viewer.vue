/* * @Author: zhangzc * @Email: zhangzc2@guahao.com * @Date: 2019-04-25 14:51:53 * @Last Modified
by: qiuwt * @Last Modified time: 2020-12-26 15:43:59 * @Description: 图片查看器 */

<template>
  <div class="image-viewer-wrapper">
    <div v-if="mode !== 'none'" :style="boxStyle">
      <el-button v-if="!img" v-bind="extend" @click="handleImgPreview()">
        {{ typeof btnLabel === 'function' ? btnLabel(this) : btnLabel }}
      </el-button>
      <img v-else :src="img" :style="imgStyle" class="cell-img" @click="handleImgPreview()" />
    </div>
    <transition name="el-alert-fade">
      <section v-if="visible" class="fixed-container">
        <div v-show="loading" class="loading-wrapper">
          <i class="icon-loading" />
          <span class="text-loading">加载中...</span>
        </div>
        <div v-show="!loading" class="img-wrapper">
          <div class="img-content">
            <span :style="`transform:translate(${dx}px,${dy}px);`" class="img-content_box">
              <img
                ref="img"
          ...
          <div class="img-tooltip">
            <div class="tooltip-wrapper">
              <div v-if="currentIds.length > 1" class="tooltip-page">
                <span class="icon-prev" @click="handlePrev" />
                <span class="page-number">{{ current + 1 }}/{{ currentIds.length }}</span>
                <span class="icon-next" @click="handleNext" />
              </div>
              ...
              <div class="tooltip-rotate">
                <span class="icon-rotate" @click="handleRotate" />
                <span class="icon-download" @click="handleDownload" />
                <span class="el-icon-printer my-el-icon" @click="handlePrinter" />
              </div>
...
export default {
  ...
  methods: {
    initData(val) {
      const { currentProps, storage, regUrl, mode } = this
      this.current = 0
